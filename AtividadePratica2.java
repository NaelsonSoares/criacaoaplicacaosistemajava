// Disciplina: Criação de Aplicação e Sistema
// Professora: Lucy Mari
// Estrutura de Controle no Java

import javax.swing.*;

//class
public class AtividadePratica2{
    
    // variaveis
    public static void main(String[] args){
        int n1, n2, p=1;
        char op=0;
        String msg=" ", msgEntr="Digite 1 para produto\n Digite 2 para produtoria\n";
    
        //entrada de dados
        n1 = Integer.parseInt (JOptionPane.showInputDialog("Digite um número ineiro"));
        n2 = Integer.parseInt (JOptionPane.showInputDialog("Digite outro número ineiro"));
        op = (JOptionPane.showInputDialog(msgEntr)). charAt(0);

        //processamento
        switch(op){
            case '1':{
                if(n1>0 && n2>0){
                    p = n1 * n2;
                    msg = msg + "Produto de " + n1 + " x " + n2 + " = " + p + "\n\n"; 
                }
                break;
            }
            case '2':{
                int i=1;
                while(i<=n2){
                    p = p * n1;
                    i++;
                }
                    msg= msg + "Produtoria de " + n1 + ", " + n2 + " vezes eh: " + p + "\n\n";
                break;
            }
                default: JOptionPane.showMessageDialog(null, "Opcao invalida, calculos nao realizados");             
        }
            // saida de resultados
                if (op >= '1' && op <= '3'){ 
            
                JOptionPane.showMessageDialog(null, msg); 
                }    
        System.exit(0);
    }
}